...
The registry is a single JSON file published as a GitHub Release asset in the
[helpcore-plugins](https://github.com/doomedramen/helpcore-plugins) repository.

The core fetches it directly from the release URL. No server, no hosting costs,
...
Anyone can submit a plugin by opening a pull request to the
[helpcore-plugins](https://github.com/doomedramen/helpcore-plugins) repository
that adds a new plugin directory under `plugins/`. The registry entry is
auto-generated from the plugin's `manifest.toml` during CI.
...
- `permissions` accurately lists everything the plugin requests — do not
  under-declare
- The built `.tar.gz` package is published to the `plugins-latest` release
- For `bridge` tier: a `setup_guide` URL with clear deployment instructions

---

## Repository layout (helpcore-plugins)

```
plugins/
  <id>/               ← plugin source (manifest.toml, tools.json, skill.md, Rust code)
    Cargo.toml
    manifest.toml
    tools.json
    skill.md
    src/
registry/
  schema.json          ← JSON Schema validating the registry format
```

The `plugins.json` file is auto-generated by CI and published as a release asset
on the `plugins-latest` release. It is no longer tracked in the repository.

## CI pipeline (helpcore-plugins)

Every push to `main` that touches `plugins/` triggers:

1. Build all WASM plugins and package them into `dist/<id>.tar.gz`
2. Publish packages to the `plugins-latest` GitHub Release
3. Generate `plugins.json` from all `manifest.toml` files
4. Upload `plugins.json` as a release asset on `plugins-latest`

Merging to `main` is the only publish mechanism — there is no separate publish
step.
...
1. Bump the `version` field in the plugin's `manifest.toml`
2. Push to `main` (or open a PR that targets `main`)
3. CI builds the new package, publishes it, and updates `plugins.json` with the
   new version, SHA-256, and package URL
