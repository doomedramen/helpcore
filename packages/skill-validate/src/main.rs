...
fn has_concrete_nouns(brief: &str) -> bool {
    // Brief should mention specific domain objects, not vague terms
    let lower = brief.to_lowercase();
    let has_specific = lower.contains("temperature") || lower.contains("forecast")
        || lower.contains("currency") || lower.contains("exchange rate")
        || lower.contains("recipe") || lower.contains("meal")
        || lower.contains("movie") || lower.contains("tv show") || lower.contains("film")
        || lower.contains("stock") || lower.contains("crypto")
        || lower.contains("definition") || lower.contains("pronunciation")
        || lower.contains("dns") || lower.contains("domain")
        || lower.contains("pollen") || lower.contains("allergy")
        || lower.contains("ip address") || lower.contains("isp")
        || lower.contains("pomodoro") || lower.contains("dice")
        || lower.contains("qr code")
        || lower.contains("sunrise") || lower.contains("sunset") || lower.contains("twilight")
        || lower.contains("timezone")
        || lower.contains("light") || lower.contains("thermostat") || lower.contains("smart home")
        || lower.contains("virtual machine") || lower.contains("vm") || lower.contains("container")
        || lower.contains("plex") || lower.contains("radarr") || lower.contains("sonarr") || lower.contains("seerr")
        || lower.contains("overseerr") || lower.contains("jellyseerr")
        || lower.contains("holiday") || lower.contains("public")
        || lower.contains("wikip") || lower.contains("summary")
        || lower.contains("color") || lower.contains("palette") || lower.contains("hex")
        || lower.contains("country") || lower.contains("population") || lower.contains("capital")
        || lower.contains("base64") || lower.contains("hash") || lower.contains("uuid")
        || lower.contains("audio") || lower.contains("transcribe") || lower.contains("speech")
        || lower.contains("format") || lower.contains("json") || lower.contains("yaml")
        || lower.contains("joke") || lower.contains("quote") || lower.contains("fact")
        || lower.contains("recommend") || lower.contains("similar")
        || lower.contains("nutrition") || lower.contains("calorie") || lower.contains("barcode")
        || lower.contains("image") || lower.contains("dimension")
        || lower.contains("html") || lower.contains("markdown") // data-format, markdown-utils
        || lower.contains("date") || lower.contains("weekday") || lower.contains("leap")
        || lower.contains("unit") || lower.contains("length") || lower.contains("weight")
...
fn validate(plugins: &[Plugin]) -> ValidationReport {
    ...
    let deduped_pairs: Vec<_> = ambiguity_pairs
        ...
        .filter(|pair| {
            let key = {
                let mut names = vec![pair.plugin_a.as_str(), pair.plugin_b.as_str()];
                names.sort();
                names.join("|")
            };
            seen.insert(key)
